        ...
        where T : KCPMsg, new()
    {
        ...
        public void InitSession(uint sid, Action<byte[], IPEndPoint> udpSender, IPEndPoint remotePoint)
        {
            ...
            m_handle.Recv = (byte[] buffer) => {
                buffer = Tools.DeCompress(buffer);
                T msg = Tools.DeSerialize<T>(buffer);
                if (msg != null)
                {
                    OnReciveMsg(msg);
                }
        ...
        public void SendMsg(T msg)
        {
            ...
            {
                Tools.Warn("Session Disconnected.Can not send msg.");
            }
        }
        public void SendMsg(byte[] msg_bytes)
        {
            if (IsConnected())
            {
                msg_bytes = Tools.Compress(msg_bytes);
        ...
        async void Update()
        {
            try
            {
                while (true)
                {
                    ...
                    {
                        Tools.ColorLog(KCPLogColor.Cyan, "SessionUpdate Task is Cancelled.");
                        break;
                    }
                    else
                    {
            ...
            {
                Tools.Warn("Session Update Exception:{0}", e.ToString());
            }
        }
